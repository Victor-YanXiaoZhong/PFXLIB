...
#ifdef _DEBUG
#define	VERSION_STR	_T("PB125") _T(" (Debug version - ") _T(__DATE__) _T(" ") _T(__TIME__) _T(")")
#else
#define	VERSION_STR	_T("PB125") _T(" (Release version - ") _T(__DATE__) _T(" ") _T(__TIME__) _T(")")
#endif
...
PBXRESULT PbniHash::Exists(PBCallInfo *ci)
{
	PBXRESULT pbxr = PBX_OK;
	pbstring argkey = ci->pArgs->GetAt(0)->GetString();
	LPCTSTR tszKey = m_pSession->GetString(argkey);
	hi_iterator_t *iter;
	void *key, *data;
	uint32_t keylen;
	pbboolean exists = 0;
