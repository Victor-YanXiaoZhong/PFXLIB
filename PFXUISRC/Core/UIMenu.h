...
#define		UI_MENU_MAGIC		0x2010		// 皮肤菜单项目信息标识

typedef basic_string<TCHAR> string_t;

struct UI_MENU_ITEM_INFO					// 皮肤菜单项目信息结构
{
	...
	string_t szText;						// 菜单项文本
	CImageEx * lpIconImgN;					// 未选中图标
	CImageEx * lpIconImgH;					// 选中图标
};

class CUIMenu								// 皮肤菜单封装类
{
public:
	CUIMenu(HMENU hMenu = NULL);
	~CUIMenu(void);
...
public:
	...
	CUIMenu GetSubMenu(int nPos) const;
...
private:
	UI_MENU_ITEM_INFO * GetMenuItemInfo(UINT nItem, BOOL bByPosition);	// 获取指定菜单项信息数据函数
	BOOL SetMenuItemInfo(UINT nItem, BOOL bByPosition, UI_MENU_ITEM_INFO * lpItemInfo);	// 设置指定菜单顶信息数据函数
